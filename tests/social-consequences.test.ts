...
describe('social consequences', () => {
  test('helpful talk changes relationship axes without forcing a quest score reward', () => {
    const world = fixture();
    const before =
      world.npcs.find((npc) => npc.id === 'mira')!.relationshipAxes?.['player']?.trust ?? 0;
    ...
    expect(
      world.npcs.find((npc) => npc.id === 'mira')!.relationshipAxes?.['player']?.trust
  ...
  test('gossip raises suspicion toward the subject and leaves a memory trail', () => {
    const world = fixture();
    const lenaBefore =
      world.npcs.find((npc) => npc.id === 'lena')!.relationshipAxes?.['pax']?.suspicion ?? 0;
  ...
  test('quest completion can leave a wary aftermath when suspicion is higher than trust', () => {
    ...
    applyAction(world, {
      ...
      actorId: 'player',
      targetId: 'mira',
      text: 'You hide bridge details and blame others.',
    });
    applyAction(world, {
      type: 'confront',
      actorId: 'player',
      targetId: 'mira',
      text: 'You are still hiding something about the missing tools.',
    });
    applyAction(world, { type: 'accept_quest', actorId: 'player', questId: 'return_shears' });
    applyAction(world, { type: 'move', actorId: 'player', locationId: 'forge' });
    applyAction(world, { type: 'pickup', actorId: 'player', itemId: 'shears' });
