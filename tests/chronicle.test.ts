...
describe("chronicle: causal trace of legible beats", () => {
  ...
  it("full causal chain: player_word → gossip → secret_revealed → turned_against → confrontation, all playerCaused", async () => {
    ...
    holder.secrets = [
      { id: "s1", text: "Mira hides the stolen moonmint shipment under the garden shed", risk: 80, knownBy: [] },
    ];
    ...
    const turned = confrontation!.causeIds.map((id) => byId.get(id)).find((event) => event?.kind === "turned_against");
    expect(turned).toBeDefined();
    const secret = turned!.causeIds.map((id) => byId.get(id)).find((event) => event?.kind === "secret_revealed");
    expect(secret).toBeDefined();
    // the secret was triggered by a gossip memory whose chronicle root is the player_word
    const gossipOrPlayer = secret!.causeIds.map((id) => byId.get(id)).find((event) => event !== undefined);
