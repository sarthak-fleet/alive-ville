...
describe("LLM proposer", () => {
  test("returns empty when LLM disabled", async () => {
    ...
    const propose = createLlmProposer({
      propose: (async () => ({ action: { type: "remember", text: "x", reason: "y" } })) as ProposeFn,
    });
    expect(await propose(fixture())).toEqual([]);
  });

  test("parses, validates, and forces actorId per NPC", withLlmEnv(async () => {
    const propose = createLlmProposer({
      maxNpcs: 2,
      propose: (async () => ({ action: { type: "remember", text: "check garden gate", reason: "vigilance" } })) as ProposeFn,
    });
  ...
  test("drops invalid actions and skip", withLlmEnv(async () => {
    const responses = [
  ...
  test("skips when router returns error or skipped", withLlmEnv(async () => {
