...
describe('rumors about the player', () => {
  ...
  it('dialogue RUMORS ABOUT YOU block appears when NPC has player-subject memories', async () => {
    ...
    const result = await generateDialogueReply(world, tomas.id, 'Hello.', {
      complete: capturingCompleter,
    });
  ...
  it('author director receives rising-figure hint when bested_the_player NPC exists', async () => {
    ...
    let capturedUser = '';
    const capturing: typeof authorBeat extends (w: World, c: infer C) => unknown ? C : never =
      async (req) => {
  ...
  it('player victory: enemy defeat becomes player-subject rumor but no promotion', () => {
    ...
    const targetNpc = world.npcs.find(
      (npc) => !npc.combat?.defeated && npc.id !== world.player.characterId
    );
    if (!targetNpc) return;
  ...
  it('chronicle ancestry: player_witnessed → gossip all playerCaused', () => {
    ...
    expect(
      mira.memories.some((m) => m.text.includes('patrol chief') && m.meta?.subject === 'player')
    ).toBe(true);
    expect(
      tomas.memories.some((m) => m.text.includes('patrol chief') && m.meta?.subject === 'player')
    ).toBe(true);
