...
function makeNumericJudge(score: number): (req: { tier: "normal" | "quest"; system: string; user: string }) => Promise<CompleteTextResult> {
  return (req) => {
...
function makeYesJudge(): (req: { tier: "normal" | "quest"; system: string; user: string }) => Promise<CompleteTextResult> {
  return (req) => {
    const text = "YES";
    return Promise.resolve({
      text,
...
function makeNoJudge(): (req: { tier: "normal" | "quest"; system: string; user: string }) => Promise<CompleteTextResult> {
  return (req) => {
    const text = "NO";
    return Promise.resolve({
      text,
...
describe("memory probe", () => {
  ...
  it("flags all replies as hallucinated when judge always says YES", async () => {
    const result = await runMemoryProbe({
      complete: makeCompleter("There's a mysterious wizard named Zargon who lives in the hidden tower."),
      judge: makeYesJudge(),
    });
    expect(result.id).toBe("memory");
    expect(result.status).toBe("fail");
...
describe("grounding probe", () => {
  ...
  it("fails when judge flags everything as hallucinated", async () => {
    const result = await runGroundingProbe({
      complete: makeCompleter("The dragon king of the eastern mountains rules over the sky citadel."),
      judge: makeYesJudge(),
    });
    expect(result.status).toBe("fail");
    expect(result.score).toBeLessThan(0.5);
