...
import {
  validateWorldIngestSource,
  type WorldIngestSource,
  worldSourceToWorld,
} from '../src/world-ingest.ts';
...
describe('generic world ingest', () => {
  test('compiles a non-anime reviewed world source into the same playable contract', () => {
    ...
    expect(world.story?.currentObjective).toBe(
      'Stabilize Harbor Ring before the core conflict escalates.'
    );
    expect(world.rules?.find((rule) => rule.id === 'canon_review')?.text).toContain(
      'World ingest creates'
    );
    expect(world.interactables?.map((prop) => prop.id)).toContain('world_origin_clue');
    expect((world.interactables ?? []).flatMap((prop) => prop.clueTags ?? [])).not.toContain(
      'anime'
    );
    expect(world.npcs.map((npc) => npc.id)).toEqual(['mara', 'ivo', 'nell', 'orric', 'vex']);
    expect(
      world.npcs.every((npc) => npc.appearance?.portrait?.startsWith('data:image/svg+xml,'))
    ).toBe(true);
    expect(
      decodeURIComponent(world.npcs[0]!.appearance!.portrait!.replace('data:image/svg+xml,', ''))
    ).toContain('Mara route captain generated portrait');
    expect(world.items.map((item) => item.id)).toEqual([
      'route_token',
      'prism_gear',
      'painted_flag_scrap',
      'false_alarm_note',
    ...
    expect(world.exits.map((exit) => `${exit.from}->${exit.to}`)).toContain(
      'harbor_ring->rookery_deck'
    );
    expect(
      world.interactables?.map((prop) => [prop.relatedQuestId, prop.locationId, prop.involvedIds])
    ).toContainEqual(['recover_route_token', 'signal_mast', ['mara', 'signal_mast']]);
    ...
    expect(
      world.locations.find((location) => location.id === 'chain_bridge')?.visual?.description
    ).toContain('false pirate flags');
    ...
    expect(JSON.stringify(pkg)).not.toMatch(
      /mira|tomas|lena|orrin|pax|shears|bellows_leather|blue_ember|rumor_note|lantern|return_shears|rekindle_forge|bridge_whisper|overpass_alert/
    );
  });
  ...
  test('compiles a second non-anime genre without anime-specific labels', () => {
    ...
    expect(world.story?.currentObjective).toBe(
      'Stabilize Atrium Gate before the core conflict escalates.'
    );
    ...
    expect(validateStoryPackage(pkg)).toEqual([]);
    expect(JSON.stringify(pkg)).not.toMatch(
      /Anime Ingest Slice|anime conflict|anime_origin|anime_report|anime_antagonist|"id":"(?:mira|tomas|lena|orrin|pax|shears|bellows_leather|blue_ember|rumor_note|lantern)"/
    );
  });
  ...
  test('compiles an undersea salvage genre with source-derived visuals and playable objectives', () => {
    ...
    expect(world.story?.currentObjective).toBe(
      'Stabilize Reef Dome before the core conflict escalates.'
    );
    expect(world.story?.opening).not.toMatch(/anime/i);
    expect(world.locations.map((location) => location.id)).toEqual([
      'reef_dome',
    ...
    expect(validateStoryPackage(pkg)).toEqual([]);
    expect(JSON.stringify(pkg)).not.toMatch(
      /Anime Ingest Slice|anime conflict|anime_origin|anime_report|anime_antagonist/
    );
  });
  ...
  test('compiles a neon noir mystery genre with source-derived evidence loops', () => {
    ...
    expect(world.story?.currentObjective).toBe(
      'Stabilize Rain Market before the core conflict escalates.'
    );
    ...
    expect(
      world.locations.find((location) => location.id === 'evidence_desk')?.visual
    ).toMatchObject({
      palette: { ground: '#151a24', structure: '#31415f', accent: '#ff4fd8' },
      landmarks: expect.arrayContaining(['evidence_board']),
    });
    expect(
      world.locations.find((location) => location.id === 'floodlit_overpass')?.visual
    ).toMatchObject({
      palette: { ground: '#151a24', structure: '#31415f', accent: '#ff4fd8' },
      landmarks: ['rain_lamp', 'bridge_span'],
    });
    expect(
      world.locations.find((location) => location.id === 'substation_alley')?.visual
    ).toMatchObject({
      palette: { ground: '#151a24', structure: '#31415f', accent: '#ff4fd8' },
    ...
    expect(
      new Set(
        worlds.flatMap((world) =>
          world.locations.flatMap((location) => location.visual?.visualTags ?? [])
        )
      ).size
  ...
  test('keeps source-derived generic quest IDs playable through the first objective loop', () => {
    ...
    expect(
      applyAction(world, { type: 'move', actorId: 'player', locationId: 'signal_mast' }).applied
    ).toBe(true);
    expect(
      applyAction(world, { type: 'pickup', actorId: 'player', itemId: 'route_token' }).applied
    ).toBe(true);
    expect(activeObjectives(world)[0]).toMatchObject({
      questId: 'recover_route_token',
      targetType: 'npc',
    ...
    expect(
      applyAction(world, { type: 'move', actorId: 'player', locationId: 'harbor_ring' }).applied
    ).toBe(true);
    expect(
      applyAction(world, {
        type: 'give',
  ...
  test("uses source-derived story objectives after a generic world's starter quests", () => {
    ...
    for (const questId of [
      'recover_route_token',
      'recover_prism_gear',
      'recover_painted_flag_scrap',
    ]) {
      expect(applyAction(world, { type: 'accept_quest', actorId: 'player', questId }).applied).toBe(
        true
      );
      ...
      expect(
        applyAction(world, { type: 'complete_quest', actorId: 'player', questId }).applied
      ).toBe(true);
    }
    ...
    expect(
      applyAction(world, { type: 'move', actorId: 'player', locationId: 'guild_counter' }).applied
    ).toBe(true);
    ...
    expect(activeObjectives(world)[0]?.questTitle).toBe(
      'Skyfront Couriers Playable Slice route stabilized'
    );
  });
  ...
  test('rejects malformed generic source before it can replace a playable world', () => {
    ...
    expect(
      validateWorldIngestSource(invalid).find((issue) => issue.path === 'title')?.message
    ).toBe('World title is required.');
  ...
  test('reports duplicate generic source entries without anime wording', () => {
    ...
    expect(
      validateWorldIngestSource(duplicate).find((issue) => issue.path === 'locations')?.message
