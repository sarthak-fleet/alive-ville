...
describe('interactable props', () => {
  test('inspect requires co-location and marks clues as inspected', () => {
    const world = fixture();
    const far = applyAction(world, {
      type: 'inspect',
      actorId: 'player',
      propId: 'forge_tool_rack',
    });
    ...
    expect(ok.applied).toBe(true);
    expect(world.interactables?.find((prop) => prop.id === 'forge_tool_rack')?.inspected).toBe(
      true
    );
