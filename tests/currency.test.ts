...
describe("currency", () => {
  ...
  test("completing a quest by gift rewards the player coins", () => {
    const world = fixture();
    expect(applyAction(world, { type: "accept_quest", actorId: "player", questId: "return_shears" }).applied).toBe(true);
    expect(applyAction(world, { type: "move", actorId: "player", locationId: "forge" }).applied).toBe(true);
    expect(applyAction(world, { type: "pickup", actorId: "player", itemId: "shears" }).applied).toBe(true);
    expect(applyAction(world, { type: "move", actorId: "player", locationId: "square" }).applied).toBe(true);
    expect(applyAction(world, { type: "move", actorId: "player", locationId: "garden" }).applied).toBe(true);

    const before = coinsOf(world, "player");
    expect(applyAction(world, { type: "give", actorId: "player", itemId: "shears", targetId: "mira" }).applied).toBe(true);
  ...
  test("buy transfers the item and moves coins both ways", () => {
    ...
    const result = applyAction(world, { type: "buy", actorId: "player", itemId: "shears", fromId: "mira" });
  ...
  test("sell moves the item to the buyer and pays the seller", () => {
    ...
    const result = applyAction(world, { type: "sell", actorId: "player", itemId: "shears", toId: "mira" });
    expect(result.applied).toBe(true);
    expect(getItem(world, "shears")!.holderId).toBe("mira");
