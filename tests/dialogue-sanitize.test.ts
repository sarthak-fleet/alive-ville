...
describe('sanitizeReply', () => {
  ...
  test("strips the npc's own name prefix", () => {
    expect(sanitizeReply('Old Doran: Hello there, traveler.', 'Old Doran', 'Wanderer')).toBe(
  ...
  test('strips a generic Player:/You: label too', () => {
    expect(sanitizeReply('Player: what brings you here?', 'Old Doran', 'Wanderer')).toBe(
      'what brings you here?'
    );
  });

  test('strips wrapping quotes and keeps a clean line', () => {
    expect(sanitizeReply('"Just here to talk."', 'Old Doran', 'Wanderer')).toBe(
      'Just here to talk.'
    );
  });
