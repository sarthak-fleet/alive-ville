...
describe('One Punch Man local test world', () => {
  test('loads through the shared engine and story package shape', () => {
    ...
    expect(engine.state.npcs.map((npc) => npc.appearance?.portrait).filter(Boolean)).toHaveLength(
      5
    );
  ...
  test('tracks Sonic combat state before resolving the OPM confrontation', () => {
    ...
    expect(world.npcs.find((npc) => npc.id === 'pax')?.combat).toMatchObject({
      hp: 58,
      defeated: false,
    });
    ...
    expect(
      world.npcs
        .find((npc) => npc.id === 'pax')
        ?.ambitions?.every((ambition) => ambition.status === 'abandoned')
    ).toBe(true);
    expect(world.npcs.find((npc) => npc.id === 'lena')?.memories.at(-1)?.text).toContain(
      'cleared the immediate threat'
    );
  ...
  test('hostile agents wait for the player to start combat before auto-engaging', async () => {
    ...
    expect(
      response.actions.filter(
        (entry) => entry.action.type === 'fight' && entry.action.targetId === 'player'
      )
    ).toHaveLength(0);
  ...
  test('lets Mumen Rider assist as a witness during the Sonic fight', async () => {
    ...
    const assist = firstExchange.actions.find(
      (entry) => entry.action.type === 'confront' && entry.action.actorId === 'lena'
    );
    expect(assist?.action).toMatchObject({ type: 'confront', actorId: 'lena', targetId: 'pax' });
    expect(assist?.text).toContain('Mumen Rider');
    expect(engine.state.npcs.find((npc) => npc.id === 'lena')?.memories.at(-1)?.text).toContain(
      'Witness assist'
    );
  ...
  test('keeps the active fight target in place during a player attack tick', async () => {
    ...
    expect(
      response.actions.some(
        (entry) => entry.action.type === 'move' && entry.action.actorId === 'pax'
      )
    ).toBe(false);
    expect(engine.state.npcs.find((npc) => npc.id === 'pax')?.locationId).toBe('square');
    expect(
      response.actions.some(
        (entry) => entry.action.type === 'move' && entry.action.actorId === 'tomas'
      )
    ).toBe(true);
  });
  ...
  test('does not let selected avatar movement pull the player out of combat', async () => {
    const engine = createEngine(fixture(), {
      propose: async () => [{ type: 'move', actorId: 'orrin', locationId: 'garden' }],
    });
  ...
  test('pins the fight target to the player location after combat scheduling', async () => {
    const engine = createEngine(fixture(), {
      propose: async () => [{ type: 'move', actorId: 'player', locationId: 'garden' }],
    });
