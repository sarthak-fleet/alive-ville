...
describe('learned quest hints', () => {
      ...
      actions: [
        {
          action: {
            type: 'remember',
            actorId: 'lena',
            text: 'Director clue: A blue pulse runs from the bridge toward every missing metal object.',
          },
  ...
  test('Z-City hints advance once the player holds the quest item', () => {
    ...
    const hints = questHintsFor(world, getQuest(world, 'return_shears')!);
    expect(hints.map((hint) => hint.text).join(' ')).toMatch(
      /Bring it back to Saitama at the Apartment Block/
    );
  });
