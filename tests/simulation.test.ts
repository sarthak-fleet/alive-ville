...
describe("simulation", () => {
  ...
  test("player actions create memories retrievable by later dialogue context", async () => {
    ...
    await engine.tick({ type: "talk", targetId: "lena", text: "The bridge is unsafe after sundown." });
  ...
  test("gossip emerges from initial world state and changes third-party trust", async () => {
    ...
    expect(event.actions.some((entry) => entry.action.type === "gossip" && entry.action.type === "gossip" && entry.action.aboutId === "pax")).toBe(true);
