...
export function storyPackageFromWorld(world: World, cutscenes: CutsceneManifestEntry[] = []): StoryPackage {
  const storyId = cutscenes.find((entry) => entry.worldId === world.id)?.storyId ?? slugId(world.story?.title ?? world.id);
  return {
    packageVersion: 1,
    worldId: world.id,
    storyId,
...
export function worldFromStoryPackage(pkg: StoryPackage): World {
  ...
  if (issues.length > 0) {
    throw new Error(`Invalid story package: ${issues.map((issue) => `${issue.path}: ${issue.message}`).join("; ")}`);
  }
...
export function validateStoryPackage(pkg: StoryPackage): StoryPackageIssue[] {
  ...
  if (pkg.packageVersion !== 1) issues.push({ path: "packageVersion", message: "Only packageVersion 1 is supported." });
  ...
  if (pkg.world.id !== pkg.worldId) issues.push({ path: "world.id", message: "world.id must match package worldId." });
  ...
  addDuplicateIssues(issues, "world.locations", pkg.world.locations.map((location) => location.id));
  addDuplicateIssues(issues, "world.npcs", pkg.world.npcs.map((npc) => npc.id));
  addDuplicateIssues(issues, "world.items", pkg.world.items.map((item) => item.id));
  addDuplicateIssues(issues, "world.interactables", pkg.world.interactables.map((prop) => prop.id));
  addDuplicateIssues(issues, "world.quests", pkg.world.quests.map((quest) => quest.id));
  addDuplicateIssues(issues, "assets.cutscenes", pkg.assets.cutscenes.map((cutscene) => cutscene.id));

  for (const exit of pkg.world.exits) {
    if (!locationIds.has(exit.from)) {
      issues.push({ path: `world.exits.${exit.from}->${exit.to}.from`, message: "Exit source location must exist in the package world." });
    }
    if (!locationIds.has(exit.to)) {
      issues.push({ path: `world.exits.${exit.from}->${exit.to}.to`, message: "Exit target location must exist in the package world." });
    }
  }
  for (const npc of pkg.world.npcs) {
    if (!locationIds.has(npc.locationId)) {
      issues.push({ path: `world.npcs.${npc.id}.locationId`, message: "NPC location must exist in the package world." });
    }
  }
  for (const item of pkg.world.items) {
    if (item.locationId && !locationIds.has(item.locationId)) {
      issues.push({ path: `world.items.${item.id}.locationId`, message: "Item location must exist in the package world." });
    }
    if (item.holderId && item.holderId !== "player" && !npcIds.has(item.holderId)) {
      issues.push({ path: `world.items.${item.id}.holderId`, message: "Item holder must be player or an NPC in the package world." });
    }
  }
  for (const prop of pkg.world.interactables) {
    if (!locationIds.has(prop.locationId)) {
      issues.push({ path: `world.interactables.${prop.id}.locationId`, message: "Interactable location must exist in the package world." });
    }
    if (prop.relatedQuestId && !questIds.has(prop.relatedQuestId)) {
      issues.push({ path: `world.interactables.${prop.id}.relatedQuestId`, message: "Interactable related quest must exist in the package quest list." });
    }
    for (const involvedId of prop.involvedIds ?? []) {
      if (!worldRefs.has(involvedId)) {
        issues.push({ path: `world.interactables.${prop.id}.involvedIds`, message: `Interactable involved id "${involvedId}" must reference a known NPC, item, location, or player.` });
      }
    }
  }
  for (const quest of pkg.world.quests) {
    if (quest.giverId && !npcIds.has(quest.giverId)) {
      issues.push({ path: `world.quests.${quest.id}.giverId`, message: "Quest giver must exist in the package NPC list." });
    }
  }
  for (const tension of pkg.world.tensions) {
    for (const involvedId of tension.involvedIds ?? []) {
      if (!worldRefs.has(involvedId)) {
        issues.push({ path: `world.tensions.${tension.id}.involvedIds`, message: `Tension involved id "${involvedId}" must reference a known NPC, item, location, or player.` });
      }
    }
  }
  for (const plan of pkg.world.villainPlans) {
    if (!npcIds.has(plan.actorId)) {
      issues.push({ path: `world.villainPlans.${plan.id}.actorId`, message: "Villain plan actor must exist in the package NPC list." });
    }
  }
  for (const cutscene of pkg.assets.cutscenes) {
    if (cutscene.worldId !== pkg.worldId) {
      issues.push({ path: `assets.cutscenes.${cutscene.id}.worldId`, message: "Cutscene worldId must match package worldId." });
    }
    if (!cutsceneIds.has(cutscene.id)) {
      issues.push({ path: `assets.cutscenes.${cutscene.id}.id`, message: "Cutscene id is required." });
    }
    if (!cutscene.src) {
      issues.push({ path: `assets.cutscenes.${cutscene.id}.src`, message: "Cutscene src is required." });
    }
    if (!cutscene.poster) {
      issues.push({ path: `assets.cutscenes.${cutscene.id}.poster`, message: "Cutscene poster is required." });
    }
  }
  return issues;
}
