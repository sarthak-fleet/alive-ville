...
export async function fetchResearchSource(
  ...
  }: Pick<SummarizeUrlOptions, 'fetcher' | 'maxChars'> = {}
  ...
  const text = contentType.includes('html')
    ? extractReadableText(body, maxChars)
    : normalizeText(body).slice(0, maxChars);
...
export async function summarizeUrl(
  ...
  if ('skipped' in result && result.skipped)
    throw new Error(`Research model skipped: ${result.reason}`);
...
export function buildResearchPrompt(
  ...
  return [
    ...
    question
      ? `Question: ${question}`
      : 'Question: Extract what matters for a future playable world/import pipeline.',
    '',
    'Return:',
...
export function extractTitle(input: string): string | null {
  ...
  const title = match?.[1]
    ? normalizeText(decodeBasicEntities(match[1].replace(/<[^>]+>/g, ' ')))
    : '';
  return title || null;
}
