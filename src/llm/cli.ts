...
export async function cliComplete(prompt: string): Promise<{ text?: string; error?: string }> {
  ...
  const lastMessageFile = join(tmpdir(), `aliveville-codex-${Date.now()}-${Math.random().toString(36).slice(2)}.txt`);
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0 && !text) void finish({ error: `${backend} exited ${code}: ${stderr.slice(0, 200)}` });
