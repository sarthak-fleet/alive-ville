...
export function createLlmProposer({
  ...
  return async function llmProposer(world: World): Promise<Action[]> {
    ...
    const proposals = await Promise.all(
      candidates.map((npc) => proposeOne(world, npc, tier, propose))
    );
...
function buildContext(world: World, npc: Npc): string {
  ...
  const memories =
    retrieveMemories(world, npc.id, recentKeywords(world), MEMORY_LIMIT)
      .map((memory) => `- (t${memory.tick}) ${memory.text}`)
      .join('\n') || '(none retrieved)';
  const relationships =
    Object.entries(npc.relationships ?? {})
      .map(([id, score]) => `${id}:${score}`)
      .join(', ') || '(none)';
  const relationshipAxes =
    Object.entries(npc.relationshipAxes ?? {})
      .map(
        ([id, axes]) =>
          `${id}:${Object.entries(axes)
            .map(([key, value]) => `${key}=${value}`)
            .join('/')}`
      )
      .join(', ') || '(none)';
  const needs = npc.needs
    ? Object.entries(npc.needs)
        .map(([key, value]) => `${key}=${value}`)
        .join(', ')
    : '(unknown)';
  const mood = npc.mood
    ? `${npc.mood.emotion}; stress=${npc.mood.stress}; confidence=${npc.mood.confidence}; suspicion=${npc.mood.suspicion}`
    : '(unknown)';
  const intent = npc.plan?.currentIntent
    ? `${npc.plan.currentIntent.kind}: ${npc.plan.currentIntent.reason}`
    : '(none)';
  const ambitions =
    (npc.ambitions ?? [])
      ...
      .map(
        (goal) =>
          `${goal.id}:${goal.title} priority=${goal.priority}${goal.blocker ? ` blocker=${goal.blocker}` : ''}`
      )
      .join('\n') || '(none)';
  const knownSecrets =
    (npc.secrets ?? [])
  ...
  const recentLog =
    (world.eventLog ?? [])
      .slice(-RECENT_TICKS)
      .flatMap((entry) => entry.actions.map((a) => `t${entry.tick}: ${a.text}`))
      .join('\n') || '(quiet)';
...
function recentKeywords(world: World): string {
  ...
  return (
    recent.flatMap((entry) => entry.actions.map((a) => a.text ?? '')).join(' ') ||
    'village rumor trust'
  );
}
