...
const AGENT_LOOP_MAX_TICKS = process.env['AGENT_LOOP_MAX_TICKS']
  ? Number(process.env['AGENT_LOOP_MAX_TICKS'])
  : null;
const AGENT_LOOP_MAX_CHECKPOINTS = Number(process.env['AGENT_LOOP_MAX_CHECKPOINTS'] ?? 24);
const AGENT_LOOP_CHECKPOINT_PATH = new URL(
  process.env['AGENT_LOOP_CHECKPOINT_FILE'] ?? './tmp/agent-loop-checkpoints.json',
  CWD
);
...
function loadAutosave(sessionId: string): { world: World; savedAt: number } | null {
  ...
  try {
    ...
    const raw = JSON.parse(readFileSync(path, 'utf8')) as
      | { savedAt?: number; world?: World }
      | World;
    const world =
      raw && typeof raw === 'object' && 'world' in raw && raw.world ? raw.world : (raw as World);
    const savedAt =
      raw && typeof raw === 'object' && 'savedAt' in raw && typeof raw.savedAt === 'number'
        ? raw.savedAt
        : Date.now();
    if (!world || typeof world !== 'object' || !('npcs' in world)) return null;
    console.info(
      `[autosave] session "${sessionId}" resuming "${world.name}" at tick ${world.tick}`
    );
...
function flushSession(session: GameSession): void {
  ...
  try {
    ...
    writeFileSync(
      `${path}.tmp`,
      JSON.stringify({ savedAt: Date.now(), world: session.engine.state })
    );
...
function createSession(id: string): GameSession {
  ...
  const propose = isLlmEnabled()
    ? createLlmProposer({ tier: 'normal', maxNpcs: LLM_MAX_NPCS })
    : undefined;
  ...
  const initialCheckpoints =
    id === 'main'
      ? readAgentLoopCheckpoints(AGENT_LOOP_CHECKPOINT_PATH).filter(
          (checkpoint) => checkpoint.world.id === world.id
        )
      : [];
...
async function sessionFor(req: IncomingMessage, url: URL): Promise<GameSession> {
  ...
  if (awayMs > 10 * 60_000) {
    ...
    if (recap) {
      session.dirty = true;
      console.info(
        `[catch-up] session "${session.id}": ${recap.ticks} ticks while away (${Math.round(awayMs / 60000)} min)`
      );
    }
  }
...
setInterval(() => {
  ...
  for (const session of sessions.values()) {
    ...
    if (
      ...
      idleFor > LOOP_IDLE_STOP_MS &&
      session.agentLoop.status().state === 'running'
    ) {
...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/worlds/select' && req.method === 'POST') {
    ...
    try {
      const raw = JSON.parse(readFileSync(new URL(entry.file, CWD), 'utf8')) as Record<
        string,
        unknown
      >;
      const nextWorld =
        entry.kind === 'source' ? worldSourceToWorld(raw as never) : (raw as unknown as World);
    ...
    try {
      ...
      if (!pkg || typeof pkg !== 'object' || !('packageVersion' in pkg)) {
        return json(res, 400, {
          ...
          issues: [{ path: 'packageVersion', message: 'Story package is required.' }],
        });
      }
      const issues = validateStoryPackage(pkg as never);
      if (issues.length > 0) return json(res, 400, { error: 'invalid_story_package', issues });
      const agentLoopStatus = await replaceEngineState(
        session,
        worldFromStoryPackage(pkg as never)
      );
      return json(res, 200, { ok: true, state: engine.state, agentLoopStatus });
    } catch (error) {
      return json(res, 400, { error: (error as Error).message });
  ...
  if (url.pathname === '/api/import-fandom' && req.method === 'POST') {
    ...
    try {
      ...
      const agentLoopStatus = await replaceEngineState(
        session,
        worldSourceToWorld(imported.source)
      );
      return json(res, 200, {
        ok: true,
        state: engine.state,
  ...
  if (
    (url.pathname === '/api/import-world-source' || url.pathname === '/api/import-anime') &&
    req.method === 'POST'
  ) {
    if (rateLimited(session, 'replace_world')) return json(res, 429, { error: 'rate_limited' });
    const body = await readJson(req).catch(() => null);
    try {
      ...
      const agentLoopStatus = await replaceEngineState(
        session,
        worldSourceToWorld(source as never)
      );
  ...
  if (url.pathname === '/api/agent-loop/restore-checkpoint' && req.method === 'POST') {
    ...
    try {
      const tick =
        body && typeof body === 'object' && typeof (body as { tick?: unknown }).tick === 'number'
  ...
  if (url.pathname === '/api/dialogue/choose' && req.method === 'POST') {
    const body = await readJson(req).catch(() => null);
    const npcId =
      body && typeof body === 'object' ? (body as { npcId?: unknown }).npcId : undefined;
    const optionId =
      body && typeof body === 'object' ? (body as { optionId?: unknown }).optionId : undefined;
    if (typeof npcId !== 'string' || typeof optionId !== 'string')
      return json(res, 400, { error: 'npcId and optionId required' });
    ...
    if (reply.action) {
      broadcastSse(session, 'tick', {
        summary: {
          ...
          actions: [
            {
              action: { type: reply.action.type, actorId: npcId, targetId: 'player' },
  ...
  if (url.pathname === '/api/dialogue' && req.method === 'POST') {
    ...
    if (body && typeof body === 'object' && (body as { stream?: unknown }).stream) {
      ...
      let streamResult = await generateDialogueReply(
        engine.state,
        npcId,
        text.trim().slice(0, 500),
        { onToken, historyKey, signal: abortController.signal }
      );
      if (
        !streamResult.ok &&
        !tokensSent &&
        !['unknown_npc', 'npc_defeated', 'npc_not_here', 'cancelled'].includes(streamResult.reason)
      ) {
        streamResult = await generateDialogueReply(engine.state, npcId, text.trim().slice(0, 500), {
          onToken,
          historyKey,
      ...
      if (streamResult.ok && streamResult.action) {
        broadcastSse(session, 'tick', {
          summary: {
            ...
            actions: [
              {
                action: { type: streamResult.action.type, actorId: npcId, targetId: 'player' },
                text: streamResult.action.text,
              },
            ],
            rejected: [],
      ...
      const done = streamResult.ok
        ? {
            ...
            action: streamResult.action ?? null,
            relationship: streamResult.relationship,
          }
...
server.listen(PORT, () => {
  console.info(
    `${mainSession.engine.state.name} running at http://localhost:${PORT} (${isLlmEnabled() ? 'LLM' : 'scripted'} mode)`
  );
});
...
function listBundledWorlds(): BundledWorld[] {
  ...
  const scan = (dir: string) => {
    ...
    for (const file of files) {
      try {
        const raw = JSON.parse(readFileSync(new URL(`${dir}${file}`, CWD), 'utf8')) as Record<
          string,
          unknown
        >;
        ...
        entries.push({
          ...
          blurb: String(
            raw['synopsis'] ?? (raw['story'] as { premise?: string } | undefined)?.premise ?? ''
          ).slice(0, 160),
