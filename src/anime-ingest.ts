...
const QUEST_SLOTS = [
  {
    ...
    giverSlot: 'mira',
    itemSlot: 0,
    fallback: 'first proof token',
  },
  {
    questId: 'rekindle_forge',
    itemId: 'bellows_leather',
    giverSlot: 'tomas',
    itemSlot: 1,
    fallback: 'repair component',
  },
  {
    questId: 'bridge_whisper',
    itemId: 'blue_ember',
    giverSlot: 'lena',
    itemSlot: 2,
    fallback: 'threat proof',
  },
] as const;
...
export function validateAnimeIngestSource(source: AnimeIngestSource): AnimeIngestIssue[] {
  ...
  addDuplicateIssues(
    issues,
    'locations',
    source.locations.map((location) => location.name)
  );
  addDuplicateIssues(
    issues,
    'characters',
  ...
  addDuplicateIssues(
    issues,
    'artifacts',
    (source.artifacts ?? []).map((artifact) => artifact.name)
  );
  return issues;
}
...
export function animeSourceToWorld(source: AnimeIngestSource): World {
  ...
  if (issues.length > 0) {
    throw new Error(
      `Invalid anime ingest source: ${issues.map((issue) => `${issue.path}: ${issue.message}`).join('; ')}`
    );
  }
  ...
  const characters = Array.from({ length: castSize }, (_, index) =>
    characterToNpc(
      source,
      index < CHARACTER_SLOTS.length
        ? CHARACTER_SLOTS[index]!
        : slugId(source.characters[index]!.name),
      index,
    ...
    rules: [
      {
        id: 'canon_review',
        kind: 'story',
        text: 'Anime ingest creates a reviewed playable draft; canon fidelity requires human approval before release.',
      },
      {
        id: 'evidence_first',
        kind: 'social',
        text: 'NPCs react more strongly to witnessed proof than to vague claims.',
      },
      {
        id: 'pressure_loop',
        kind: 'story',
        text: 'Ignored conflicts raise director pressure and unlock confrontation objectives.',
      },
    ],
    ...
    villainPlans: [
      {
        ...
        objective:
          source.conflicts?.[0]?.objective ??
          `Push ${locations[0]!.name} into panic before proof is gathered.`,
        stage: 1,
        hidden: true,
        ...
        knownFacts: [
          source.conflicts?.[0]?.clue ??
            `${antagonist.name} left a clue near ${locations[4]!.name}.`,
        ],
      },
    ],
    directorState: {
      ...
      pendingReveals: [
        source.conflicts?.[0]?.clue ??
          `A clue points from ${locations[4]!.name} back to ${locations[0]!.name}.`,
      ],
    },
    locations,
    exits: [
    ...
    interactables: [
      {
        ...
        inspectText:
          artifacts[0]!.clue ?? `${artifacts[0]!.name} was moved after the morning routine.`,
      ...
      },
      {
        ...
        inspectText:
          artifacts[2]!.clue ?? `The report needs physical proof from ${locations[4]!.name}.`,
      ...
      },
      {
        ...
        inspectText:
          source.conflicts?.[0]?.clue ??
          `${antagonist.name} wants the conflict to become visible at ${locations[0]!.name}.`,
...
function resolveAntagonistIndex(source: AnimeIngestSource, castSize: number): number {
  ...
  if (declared) {
    const found = source.characters.findIndex(
      (draft) => draft.name.trim().toLowerCase() === declared
    );
...
function characterToNpc(
  source: AnimeIngestSource,
  id: string,
  index: number,
  ...
  return {
    ...
    appearance: {
      ...
      palette,
      silhouette:
        draft.look?.silhouette ?? `${draft.name}'s silhouette should read as ${draft.role}.`,
      visualTags,
    ...
    goals: draft.goals ?? [
      `resolve ${source.title}'s first local conflict`,
      'keep the playable slice stable',
    ],
    ambitions: (draft.goals ?? [`resolve ${source.title}'s first local conflict`])
      .slice(0, 2)
      .map((goal, goalIndex) => ({
    ...
    memories: (
      draft.memories ?? [`${draft.name} remembers the first clue from ${source.title}.`]
    ).map((memory) => ({
      tick: 0,
      text: memory,
      meta: {
...
function artifactSet(source: AnimeIngestSource): Required<AnimeArtifactDraft>[] {
  const defaults: Required<AnimeArtifactDraft>[] = [
    {
      name: 'first proof token',
      description: 'The first small item that makes the anime conflict playable.',
      location: 'forge',
      holder: '',
      clue: 'The item moved after a morning routine.',
    },
    {
      name: 'repair component',
      description: "A component needed to stabilize the second quest giver's plan.",
      location: 'wood',
      holder: '',
      clue: 'The component was lost near a dangerous route.',
    },
    {
      name: 'threat proof',
      description: 'Physical evidence that turns rumor into a valid report.',
      location: 'bridge',
      holder: '',
      clue: 'The proof sits near the pressure source.',
    },
    {
      name: 'challenge note',
      description: "A written clue that exposes the antagonist's intent.",
      location: 'bridge',
      holder: '',
      clue: 'The note points back to the public square.',
    },
    {
      name: 'signal radio',
      description: 'A local report device that makes pressure visible.',
      location: 'inn',
      holder: '',
      clue: 'The radio needs better evidence before sounding an alert.',
    },
  ];
...
function itemFromArtifact(
  id: string,
  artifact: Required<AnimeArtifactDraft>,
  locationId: string
): Item {
  return {
    id,
...
function artifactVisualFor(artifact: Required<AnimeArtifactDraft>): ItemVisualDesign {
  ...
  if (/gear|core|crystal|prism|glass|ember/.test(text)) {
    return {
      ...
      shape: 'core',
      palette: {
        primary: /ember/.test(text) ? '#f08a38' : '#9fc3ff',
        emissive: /ember/.test(text) ? '#7a2c08' : '#12304a',
      },
      visualTags,
    };
  }
...
function factionsFor(source: AnimeIngestSource) {
  const drafts = source.factions?.length
    ? source.factions
    : [
        {
          name: 'Allies',
          goals: ['protect civilians', 'resolve the first conflict'],
          resources: ['local trust'],
          reputation: 1,
        },
        {
          name: 'Challengers',
          goals: ['force confrontation', 'raise pressure'],
          resources: ['ambush routes'],
          reputation: -2,
        },
      ];
...
function tensionsFor(source: AnimeIngestSource, antagonistId: string) {
  const conflicts = source.conflicts?.length
    ? source.conflicts
    : [{ title: `${source.title} pressure is rising`, pressure: 45, involved: [antagonistId] }];
...
function relationshipsFor(id: string, isAntagonist: boolean): Record<string, number> {
  ...
  return Object.fromEntries(
    peers.map((peer, index) => [peer, isAntagonist ? -1 : Math.max(-1, 2 - index)])
  );
}
...
function goalKindFor(
  goal: string,
  isAntagonist: boolean
): 'protect' | 'investigate' | 'repair' | 'reveal' | 'harm' {
...
function locationPaletteFor(
  index: number,
  draft: AnimeLocationDraft
): { ground: string; structure: string; accent: string } {
  ...
  if (
    /neon|noir|\brain\b|precinct|evidence|confession|witness|substation|transformer|floodlit/.test(
      text
    )
  ) {
...
function locationLandmarksFor(index: number, draft: AnimeLocationDraft): string[] {
  ...
  if (/neon|flickering signs|pink|market|signal booth|police-band/.test(text))
    inferred.add('neon_sign');
  ...
  if (/reef|coral|kelp|subsea|undersea|abyss|dome/.test(text) && !hasSonar)
    inferred.add('reef_spire');
  if (hasSonar) inferred.add('sonar_dish');
  if (/\btraining\b|\bforge\b|\brepair\b|mast|tower/.test(text) && !hasSonar)
    inferred.add(index === 1 ? 'forge_chimney' : 'signal_tower');
  if (/garden|apartment|home|rookery|deck/.test(text))
    inferred.add(/apartment/.test(text) ? 'apartment_tower' : 'garden_planter');
  if (/kiosk|guild|counter|report|inn/.test(text))
    inferred.add(/guild|counter/.test(text) ? 'kiosk_sign' : 'lantern_post');
  if (/bridge|overpass|threat|chain/.test(text)) inferred.add('bridge_span');
  if (/wood|alley|engine|danger/.test(text))
    inferred.add(/engine/.test(text) ? 'engine_stack' : 'wood_tree');
  if (inferred.size === 0)
    inferred.add(
      [
        'notice_board',
...
function addDuplicateIssues(issues: AnimeIngestIssue[], path: string, names: string[]): void {
  ...
  for (const name of names) {
    ...
    if (seen.has(id))
      issues.push({ path, message: `Duplicate anime source entry "${name}" is not allowed.` });
    seen.add(id);
  }
}
