...
export function directorStoryBeatText(world: World): string | null {
  if (world.storyProgress?.phase === "nightfall_warning") {
    if (world.id === "ashbend") return "Director beat: The Lantern Inn should be reached before the fog thickens.";
  ...
  if (world.storyProgress?.phase === "shadow_confrontation") {
    if (world.id === "ashbend") return "Director beat: The lantern shadow is present; confronting it will force the night to resolve.";
...
export function quietWorldRevealText(world: World): string {
  if (world.id === "ashbend") return "The village has gone quiet enough for the bridge pattern to stand out.";
...
export function phasePressureRevealText(world: World): string | null {
  if (world.storyProgress?.phase === "nightfall_warning") {
    if (world.id === "ashbend") return "The Lantern Inn windows dim when the bridge whisper crosses the square.";
  ...
  if (world.storyProgress?.phase === "shadow_confrontation") {
    if (world.id === "ashbend") return "Lena's lantern throws one shadow too many across the inn floor.";
...
export function planRevealText(world: World, planId: string, stage: number): string {
  if (planId === "bridge_whisper_plan" && world.id === "ashbend") {
