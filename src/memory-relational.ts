...
export function memoriesAbout(memories: Memory[], entityId: string, entityName: string | undefined, limit = 3): Memory[] {
  const name = entityName?.toLowerCase();
  return memories
    .filter((memory) => memory.meta?.subject === entityId || (name ? memory.text.toLowerCase().includes(name) : false))
...
export function entitiesInText(text: string, entities: NamedEntity[]): NamedEntity[] {
  ...
  return entities.filter((entity) => entity.name.length > 0 && lower.includes(entity.name.toLowerCase()));
}
 ...
 */
export function relationalContext(memories: Memory[], query: string, player: NamedEntity, others: NamedEntity[], perEntity = 3): string {
  ...
  for (const entity of targets) {
    ...
    if (mems.length > 0) blocks.push(`What you remember about ${entity.name}:\n${mems.map((memory) => `- ${memory.text}`).join("\n")}`);
  }
  return blocks.join("\n");
}
