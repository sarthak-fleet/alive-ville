...
export async function generateDialogueReply(
  ...
  const complete: DialogueCompleter =
    options.complete ?? (options.onToken ? streamText : completeText);
  const npc = world.npcs.find((entry) => entry.id === npcId);
  ...
  if (
    ...
    !isAdjacent(world, npc.locationId, world.player.locationId)
  ) {
  ...
  const bufferingOnToken = userOnToken
    ? heldBackTokenizer((delta: string) => {
        streamBuffer += delta;
      })
    : undefined;
  ...
  const result = await complete({
    tier: npc.tier === 'quest' ? 'quest' : 'normal',
    system,
    user,
    onToken,
  });
  ...
  if (!coherence.ok) {
    ...
    const retry = await complete({
      ...
      user,
      onToken: retryOnToken,
    });
    const retryParsed =
      'text' in retry && retry.text
        ? parseDialogueJson(retry.text, npc.name, world.player.name ?? '')
        : null;
  ...
  npc.memories.push(
    ...
    {
      tick: world.tick,
      text: `I replied to ${playerName}: ${parsed.reply}`,
      meta: { visibility: 'private', importance: 2 },
    }
  );
  ...
  if (parsed.disposition !== 0) {
    ...
    npc.relationships = {
      ...npc.relationships,
      player: clamp(current + parsed.disposition, -10, 10),
    };
  ...
  if (parsed.action?.type === 'spar') {
    ...
    npc.memories.push({
      tick: world.tick,
      text: `I sparred with the player to test their resolve.`,
      meta: { importance: 2, visibility: 'private' },
    });
    ...
    if (typeof locationId === 'string') {
      ...
      if (validateAction(world, candidate).ok) {
        ...
        npc.memories.push({
          tick: world.tick,
          text: `I led the player toward ${locationName(world, locationId)}.`,
          meta: { importance: 2, visibility: 'private' },
        });
      }
    }
  ...
  return {
    ...
    relationship: relationshipFor(npc),
  };
}
...
export function buildDialogueSystem(world: World, npc: Npc): string {
  ...
  const playerRumors = npc.memories
    ...
    .map((m) => `- ${m.text}`)
    .join('\n');
  const rumorBlock = playerRumors ? [`RUMORS ABOUT YOU`, playerRumors].join('\n') : '';
  ...
  return [
    ...
    npc.mood
      ? `Current mood: ${npc.mood.emotion} (stress ${npc.mood.stress}, suspicion ${npc.mood.suspicion}).`
      : '',
...
async function buildDialogueUser(
  world: World,
  npc: Npc,
  ...
  const conversation = history
    ...
    .map((turn) =>
      turn.speaker === 'event'
        ? `[${turn.text}]`
        : `${turn.speaker === 'player' ? 'Player' : npc.name}: ${turn.text}`
    )
    .join('\n');
  ...
  if (playerCharId) {
    ...
    if (typeof charRel === 'number')
      stanceLines.push(`  Disposition toward ${playerName}: ${charRel}`);
  ...
  if (stanceLines.length === 0) {
    stanceLines.push(
      `  Generic disposition toward player: ${relationship.label} (${relationship.score})`
    );
  }
  ...
  const charMemoryHits =
    playerCharId || playerName
      ? (npc.memories ?? [])
          .filter((m) => {
            const text = m.text ?? '';
            const hitId = playerCharId
              ? text.toLowerCase().includes(playerCharId.toLowerCase())
              : false;
            const hitName = playerName
              ? text.toLowerCase().includes(playerName.toLowerCase())
              : false;
  ...
  return [
    ...
    memories
      ? `${showcase ? "What you've heard around the plaza" : 'Your other relevant memories'}:\n${memories}`
      : '',
...
function createDynamicQuest(
  ...
  if (
    quests.some(
      (quest) =>
        quest.title.toLowerCase() === title.toLowerCase() &&
        quest.status !== 'done' &&
        quest.status !== 'failed'
    )
  ) {
  ...
  const quest = {
    ...
    title,
    description:
      typeof action.description === 'string' ? action.description.trim().slice(0, 200) : undefined,
    giverId: npc.id,
  ...
  npc.memories.push({
    tick: world.tick,
    text: `I asked the player to: ${title}`,
    meta: { importance: 3, visibility: 'private' },
  });
...
function parseDialogueJson(raw: string, npcName: string, playerName = ''): ParsedDialogue {
  ...
  if (marker !== -1) {
    ...
    try {
      const control = JSON.parse(tail) as {
        action?: ParsedDialogue['action'];
        disposition?: unknown;
      };
      return {
        reply: spoken,
        ...
        disposition:
          typeof control.disposition === 'number'
            ? Math.max(-2, Math.min(2, Math.round(control.disposition)))
            : 0,
      };
    } catch {
  ...
  if (start !== -1 && end > start) {
    try {
      const parsed = JSON.parse(text.slice(start, end + 1)) as Partial<ParsedDialogue> & {
        reply?: unknown;
        disposition?: unknown;
      };
      const reply =
        typeof parsed.reply === 'string' ? sanitizeReply(parsed.reply, npcName, playerName) : '';
      const disposition =
        typeof parsed.disposition === 'number'
          ? Math.max(-2, Math.min(2, Math.round(parsed.disposition)))
          : 0;
      const action =
        parsed.action && typeof parsed.action === 'object'
          ? (parsed.action as ParsedDialogue['action'])
          : null;
  ...
  switch (action.type) {
    case 'move':
      return action.locationId
        ? { type: 'move', actorId: npcId, locationId: action.locationId }
        : null;
    case 'give':
      return action.itemId
        ? { type: 'give', actorId: npcId, itemId: action.itemId, targetId: 'player' }
        : null;
    case 'offer_quest':
      return action.questId
        ? { type: 'offer_quest', actorId: npcId, questId: action.questId, targetId: 'player' }
        : null;
    case 'fight':
      return { type: 'fight', actorId: npcId, targetId: 'player' };
    case 'complete_quest':
      return action.questId
        ? { type: 'complete_quest', actorId: npcId, questId: action.questId }
        : null;
    default:
      return null;
  }
...
function isAdjacent(world: World, a: string, b: string): boolean {
  return (world.exits ?? []).some(
    (exit) =>
      (exit.from === a && exit.to === b) ||
      (exit.bidirectional !== false && exit.from === b && exit.to === a)
  );
}
