...
export function storyDialogueOptions(world: World, npcId: string): StoryOption[] | null {
  ...
  for (const quest of questsFrom(world, npc)) {
    ...
    if (status === 'open')
      options.push({ id: `accept:${quest.id}`, label: `I'll help: ${quest.title}` });
    if (
      status === 'active' &&
      quest.acceptedBy === 'player' &&
      questObjectiveMet(world, quest) !== false
    ) {
  ...
  if (npc.combat && !npc.combat.defeated && aboutMe.score >= 0)
    options.push({ id: 'spar', label: 'Spar with me?' });
  ...
  const exit = (world.exits ?? []).find(
    (entry) =>
      entry.from === npc.locationId ||
      (entry.bidirectional !== false && entry.to === npc.locationId)
  );
  if (exit) {
...
export function storyDialogueRespond(
  world: World,
  npcId: string,
  optionId: string
): StoryReply | null {
  ...
  if (optionId === 'news') {
    ...
    reply = goal
      ? `${goal} That is what keeps me up at night.`
      : `Nothing I would burden a stranger with. Yet.`;
    ...
    reply = action
      ? `Then it is in your hands. Do not make me regret asking.`
    ...
    reply = action
      ? `You actually did it. I will not forget this.`
      : `It is not done yet — I can tell.`;
    ...
    npc.memories.push({
      tick: world.tick,
      text: `I agreed to walk with the player for a while.`,
      meta: { importance: 2, visibility: 'private' },
    });
    ...
    if (moved) {
      action = {
        type: 'lead',
        text: `${npc.name} sets off toward ${locationName(world, locationId)} — follow them!`,
      };
      reply = `Keep up, then.`;
    } else {
