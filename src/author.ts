...
export async function authorBeat(
  ...
  const applied =
    beat.kind === 'quest' && beat.quest
      ? applyQuestBeat(world, beat.quest)
      : beat.kind === 'arrival' && beat.arrival
        ? applyArrivalBeat(world, beat.arrival)
        : beat.kind === 'incident' && beat.incident
          ? applyIncidentBeat(world, beat.incident)
          : null;
  if (applied) {
...
function describeWorldForAuthor(world: World): string {
  ...
  const cast = world.npcs
    ...
    .map(
      (npc) =>
        `${npc.name} (${npc.role ?? 'resident'}) at ${locationNameOf(world, npc.locationId)} — wants: ${npc.goals?.[0] ?? '?'}`
    );
  const tensions = (world.tensions ?? []).map(
    (tension) => `${tension.title} (pressure ${tension.pressure ?? '?'})`
  );
...
function locationIdByName(world: World, name: string | undefined): string {
  if (name) {
    const hit = world.locations.find(
...
function applyQuestBeat(
  world: World,
  quest: NonNullable<BeatJson['quest']>
): AuthoredResult | null {
  if (!quest.title || !quest.giverName) return null;
  const giver =
    world.npcs.find((npc) => npc.name.toLowerCase() === quest.giverName!.toLowerCase()) ??
    world.npcs[0];
...
function applyArrivalBeat(
  world: World,
  arrival: NonNullable<BeatJson['arrival']>
): AuthoredResult | null {
  ...
  return {
    text: `A stranger arrives: ${arrival.name}, ${arrival.role ?? 'newcomer'}.`,
    focusActorId: id,
  };
}

function applyIncidentBeat(
  world: World,
  incident: NonNullable<BeatJson['incident']>
): AuthoredResult | null {
  if (!incident.text) return null;
  const locationId = locationIdByName(world, incident.locationName);
  const witnesses = world.npcs
    .filter((npc) => npc.locationId === locationId && !npc.combat?.defeated)
    .slice(0, 2);
  ...
  if (world.directorState && typeof incident.pressureDelta === 'number') {
    world.directorState.pressure = Math.max(
      0,
      Math.min(100, world.directorState.pressure + incident.pressureDelta)
    );
  }
