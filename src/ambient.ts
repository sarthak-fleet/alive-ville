...
export function ambientBarkForNpc(world: World, npc: Npc): AmbientBark | null {
  const activeQuest = (world.quests ?? []).find((quest) => quest.giverId === npc.id && quest.status === "active");
  const openQuest = (world.quests ?? []).find((quest) => quest.giverId === npc.id && (quest.status ?? "open") === "open");
  const doneQuest = (world.quests ?? []).find((quest) => quest.giverId === npc.id && quest.status === "done");
  const tod = timeOfDay(world.clock);

  const line = questLine(npc.id, activeQuest?.id, "active")
...
export function ambientBarksForLocation(world: World, locationId: string, limit = 3): AmbientBark[] {
  return world.npcs
...
function questLine(npcId: string, questId: string | undefined, state: "open" | "active" | "done"): string | null {
  ...
  if (questId === "return_shears" && npcId === "mira") return state === "open" ? "Those shears better not be in the mud." : "Forge first, then back here.";
  if (questId === "rekindle_forge" && npcId === "tomas") return state === "open" ? "Bellows leather. Dry. That is the difference." : "Bring the leather before the flame gives up.";
  if (questId === "bridge_whisper" && npcId === "lena") return state === "open" ? "No one crosses the bridge alone tonight." : "A note or an ember. Something real.";
  return null;
}
