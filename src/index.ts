...
console.info(`Entering ${engine.state.name} (${isLlmEnabled() ? "LLM" : "scripted"} mode)`);
console.info(await engine.tick({ type: "talk", targetId: "lena", text: "I heard trouble near the bridge." }));
console.info(await engine.tick({ type: "move", locationId: "garden" }));
