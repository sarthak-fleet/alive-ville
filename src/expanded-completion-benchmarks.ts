...
export function expandedCompletionReport(rootDir = process.cwd()): ExpandedCompletionReport {
  const gates = [
    ...
    gate(rootDir, "generic_world_ingest", "Generic world ingest", [
      ...
      source("tests/world-ingest.test.ts", ["Skyfront Couriers: World Ingest Slice", "world_origin_clue", "recover_route_token", "route_token", "a_false_pirate_alarm_threatens_the_harbor_route_plan", "keeps source-derived generic quest IDs playable", "World title is required.", "invalid-source.json", "rejects malformed generic source", "Invalid world ingest source"]),
