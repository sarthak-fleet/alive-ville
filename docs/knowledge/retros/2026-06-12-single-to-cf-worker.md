---
title: "Retro: Local dev server → Cloudflare Workers + Durable Objects deploy"
description: "Retro: local dev server to Cloudflare Workers + Durable Objects."
---
...
→ `docs/knowledge/learnings/lessons.md` §"Durable Object RPC patterns" covers the same-account
   service binding discovery and the debounce pattern.  
→ `docs/architecture/decisions/` ADR-004 covers the KV vs D1 vs DO choice rationale.
