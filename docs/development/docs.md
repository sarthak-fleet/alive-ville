...
See the fleet-wide Blume install pattern in the fleet workspace at
`fleet-ops/docs/blume-docs.md` (a separate repo, outside this one). Aliveville
does not yet have a published docs domain; set `deployment.site` in
`blume.config.ts` when one is chosen.
