...
export function paletteForText(text: string): Palette {
  ...
  if (/forge|training|engine|repair/i.test(text))
    return { ground: '#4a3c2c', structure: '#9c5a34', accent: '#ffa24a' };
  if (/garden|wood|rookery|home/i.test(text))
    return { ground: '#33513a', structure: '#58955d', accent: '#c4ee9a' };
  if (/bridge|overpass|alley|threat/i.test(text))
    return { ground: '#3a4654', structure: '#75839c', accent: '#8fd6ff' };
  if (/inn|kiosk|counter|report/i.test(text))
    return { ground: '#3e4356', structure: '#6c7994', accent: '#ffdf6a' };
...
function itemVisualFallback(text: string): ItemVisual {
  if (/gear|core|crystal|prism|glass|ember/i.test(text)) {
    return {
      color: '#9fc3ff',
      emissiveColor: '#12304a',
      material: 'crystal',
      shape: /gear/i.test(text) ? 'gear' : 'core',
    };
  }
  if (/flag|scrap|cloth|paint/i.test(text))
    return { color: '#e05f7a', emissiveColor: '#3a1420', material: 'cloth', shape: 'scrap' };
  if (/coupon|note|paper|map|letter/i.test(text))
    return { color: '#f7e8a5', emissiveColor: '#3d331a', material: 'paper', shape: 'note' };
  if (/radio|signal/i.test(text))
    return { color: '#596477', emissiveColor: '#7fd0ff', material: 'radio', shape: 'radio' };
  if (/scale|bone|shell|fang/i.test(text))
    return { color: '#7fd0ff', emissiveColor: '#17324a', material: 'organic', shape: 'scale' };
  if (/token|coin|brass|badge|key/i.test(text))
    return { color: '#d8a441', emissiveColor: '#3d2a05', material: 'metal', shape: 'token' };
