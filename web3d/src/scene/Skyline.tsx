 ...
 */
export const Skyline = memo(function Skyline({ model, worldId, night }: { model: WorldModel; worldId: string; night: boolean }) {
  const { bounds } = model;
  ...
  return (
    <group>
      {towers.map((tower, index) => (
        <mesh key={index} position={[tower.x, tower.h / 2, tower.z]} material={materials[tower.tone > 0.5 ? 0 : 1]}>
