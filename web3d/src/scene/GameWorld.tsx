...
export function GameWorld({ world }: { world: World }) {
  ...
  const activeInterior = interiorBuildingId
    ? interiorForBuilding(world, model, interiorBuildingId)
    : null;

  useEffect(() => {
    if (!import.meta.env.DEV || typeof window === 'undefined') return;
    const debug = (window as unknown as Record<string, unknown>)['__game'] as
  ...
  const activeDistrict =
    model.districts.find((district) => district.locationId === world.player.locationId) ??
    model.districts[0];
    ...
    >
      ...
      <Suspense fallback={null}>
        <Physics gravity={[0, -22, 0]}>
          <Lighting
            world={world}
            target={{ x: activeDistrict.courtyard.x, z: activeDistrict.courtyard.z }}
          />
          {night ? (
            <Stars
              radius={300}
          ...
          {world.npcs.map((npc) => {
            ...
            return (
              <Npc
                ...
                model={model}
                quests={world.quests ?? []}
                staticMode={world.showcase === true}
              />
            );
          })}
          <PlayerController
