...
export function drawCompass(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number): void {
  ctx.save();

  // Cardinal tick marks
  const dirs: Array<[string, number]> = [["N", 0], ["E", Math.PI / 2], ["S", Math.PI], ["W", -Math.PI / 2]];
