 ...
 */
export const CharacterModel = forwardRef<CharacterAnimationHandle, CharacterModelProps>(
  function CharacterModel({ visual, appearance, seedId }, ref) {
    ...
    useFrame((_, delta) => {
      ...
      if (leftArm.current && leftForearm.current) {
        ...
        leftForearm.current.rotation.x = walking
          ? -0.35 - Math.max(0, sinP) * 0.3
          : -0.12 - Math.sin(phase * 0.8) * 0.03;
      }
      if (rightArm.current && rightForearm.current) {
        rightArm.current.rotation.x = sinP * swing * 0.85;
        rightArm.current.rotation.z = -0.1;
        rightForearm.current.rotation.x = walking
          ? -0.35 - Math.max(0, -sinP) * 0.3
          : -0.12 - Math.sin(phase * 0.8 + 1) * 0.03;
      }
      if (torso.current) {
        // bounce, breath, and a forward lean that grows with speed
        torso.current.position.y = walking
          ? Math.abs(Math.sin(phase)) * (running ? 0.06 : 0.035)
          : Math.sin(phase * 0.7) * 0.012;
      ...
      if (head.current) {
        head.current.rotation.x = walking
          ? -(running ? 0.1 : 0.04)
          : Math.sin(phase * 0.7 + 0.6) * 0.02;
        head.current.rotation.z = walking ? cosP * 0.03 : 0;
      }
      if (cape.current) {
        cape.current.rotation.x =
          0.14 + (walking ? (running ? 0.55 : 0.3) + sinP * 0.07 : Math.sin(phase * 0.7) * 0.05);
      }
      if (ponytail.current) {
        ponytail.current.rotation.x =
          (walking ? 0.32 : 0.1) + Math.sin(phase * (walking ? 1 : 0.7)) * 0.08;
      }
      ...
      if (anim) {
        ...
        if (elapsed >= duration) {
          ...
          if (anim.kind === 'attack1' && rightArm.current && rightForearm.current) {
            ...
            for (const arm of [leftArm.current, rightArm.current])
              if (arm) arm.rotation.x = -punch * 2.3;
            for (const forearm of [leftForearm.current, rightForearm.current])
              if (forearm) forearm.rotation.x = -0.1;
      ...
      <group ref={root} scale={p.scale}>
          ...
          <group ref={head} position={[0, headCenter, 0]}>
            ...
            {[-1, 1].map((side) => (
              <group key={side} position={[side * p.headRadius * 0.4, 0.015, p.headRadius * 0.82]}>
                ...
                <mesh
                  position={[0, -0.004, 0.024]}
                  material={toonMaterial('#1d2330')}
                  scale={[1, 1.45, 0.5]}
                >
            ...
            {/* mouth */}
            <mesh
              position={[0, -p.headRadius * 0.42, p.headRadius * 0.86]}
              material={toonMaterial('#9c5a55')}
            >
            ...
            {visual.bodyShape === 'mechanical' ? (
              <mesh
                position={[0, 0.03, p.headRadius * 0.72]}
                material={toonMaterial('#2b3442', visual.accentColor)}
              >
        ...
        {([-1, 1] as const).map((side) => (
          <group
            ...
            position={[side * p.torsoWidth * 0.27, hipY, 0]}
          >
    ...
    <group>
      ...
      {style === 'ponytail' ? (
        <group ref={ponytailRef} position={[0, radius * 0.55, -radius * 0.75]}>
          <mesh
            position={[0, -radius * 0.55, -radius * 0.12]}
            rotation={[0.5, 0, 0]}
            castShadow
            material={material}
          >
      ...
      {style === 'bob' ? (
        <mesh position={[0, -radius * 0.12, -radius * 0.28]} castShadow material={material}>
          <sphereGeometry
            args={[radius * 1.04, 14, 10, 0, Math.PI * 2, Math.PI * 0.35, Math.PI * 0.36]}
          />
        </mesh>
      ) : null}
      {style === 'buns' ? (
        <>
          <mesh
            position={[radius * 0.62, radius * 0.62, -radius * 0.25]}
            castShadow
            material={material}
          >
      ...
      {style !== 'spiky' ? (
        <mesh
          position={[0, radius * 0.52, radius * 0.62]}
          rotation={[0.5, 0, 0]}
          castShadow
          material={material}
        >
