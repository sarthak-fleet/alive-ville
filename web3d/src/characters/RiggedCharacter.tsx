...
export function isFemalePresenting(
  ...
  const text =
    `${personaText} ${appearance?.sourceLook ?? ''} ${appearance?.outfit ?? ''} ${(appearance?.visualTags ?? []).join(' ')}`.toLowerCase();
  // word boundaries matter: "dressed as a gentleman" must not read as feminine
  return /\bshe\b|\bher\b|\bhers\b|\bwoman\b|\bgirl\b|\blady\b|female|feminine|\bdress\b|\bskirt\b/.test(
    text
  );
}
...
function outfitCutFor(
  text: string,
  seedId: string,
  female: boolean
): { shortSleeves: boolean; shorts: boolean } {
  const hash = stableHash(`${seedId}:cut`);
  const longSleeved = /kimono|haori|robe|suit|uniform|cloak|coat|jacket/.test(text);
  const shortSleeves =
    /sleeveless|tank|bare arms|short sleeve/.test(text) || (!longSleeved && hash % 3 === 0);
...
function hairColorFor(appearance: CharacterAppearance | undefined, seedId: string): string {
  const text =
    `${appearance?.hair ?? ''} ${appearance?.sourceLook ?? ''} ${(appearance?.visualTags ?? []).join(' ')}`.toLowerCase();
 ...
 */
export const RiggedCharacter = forwardRef<CharacterAnimationHandle, RiggedCharacterProps>(
  function RiggedCharacter(
    ...
    const { scene, mixer, actions, normalizeScale, headBone, chestBone, hipsBone } = useMemo(() => {
      ...
      return {
        ...
        hipsBone:
          cloned.getObjectByName('pelvis') ??
          cloned.getObjectByName('Hips') ??
          cloned.getObjectByName('spine_01') ??
          null,
      };
    ...
    const female = useMemo(
      () => isFemalePresenting(appearance, personaText),
      [appearance, personaText]
    );
    ...
    const decor = useMemo(() => {
      ...
      const hairStyle =
        visual.bodyShape === 'mechanical'
          ...
          : accessories.hat
            ? 'flat'
            : hairStyleFor(appearance, seedId, female);
      const head = new THREE.Group();
      buildHeadDecor(
        head,
        hairStyle,
        hairColorFor(appearance, seedId),
        visual,
        accessories,
        faceVariantFor(seedId, personaText)
      );
      ...
      const needsChest =
        visual.bodyShape === 'caped' || accessories.scarf || roleDecor.chest !== null;
      if (needsChest) {
        chest = new THREE.Group();
        if (visual.bodyShape === 'caped') {
          const cape = new THREE.Mesh(
            new THREE.BoxGeometry(0.55, 0.85, 0.03),
            softToonMaterial(visual.accentColor)
          );
        ...
        if (accessories.scarf) {
          const scarf = new THREE.Mesh(
            new THREE.CylinderGeometry(0.16, 0.18, 0.16, 10, 1, true),
            softToonMaterial('#a02828')
          );
          scarf.position.set(0, 0.16, 0);
          chest.add(scarf);
          const tail = new THREE.Mesh(
            new THREE.BoxGeometry(0.09, 0.3, 0.03),
            softToonMaterial('#a02828')
          );
      ...
      if (needsHips) {
        ...
        if (female) {
          const skirt = new THREE.Mesh(
            new THREE.CylinderGeometry(0.21, 0.34, 0.42, 12, 1, true),
            softToonMaterial(visual.accentColor)
          );
        ...
        if (accessories.sword) {
          ...
          const scabbard = new THREE.Mesh(
            new THREE.CylinderGeometry(0.028, 0.034, 0.86, 6),
            softToonMaterial('#1c1f2a')
          );
          ...
          const hilt = new THREE.Mesh(
            new THREE.CylinderGeometry(0.024, 0.024, 0.2, 6),
            softToonMaterial(visual.accentColor)
          );
    ...
    useEffect(() => {
      ...
      bindCorrection.current = {
        head: correctionFor(headBone),
        chest: correctionFor(chestBone),
        hips: correctionFor(hipsBone),
      };
    ...
    const playOverlay = (clipName: string, fadeIn: number) => {
      ...
      window.setTimeout(
        () => action.fadeOut(0.12),
        Math.max(0, action.getClip().duration * 1000 - 120)
      );
    };
    ...
    useFrame((_, rawDelta) => {
      ...
      const material = (
        scene.getObjectByProperty('isSkinnedMesh', true) as THREE.SkinnedMesh | undefined
      )?.material as THREE.MeshToonMaterial | undefined;
      if (material) {
      ...
      const targetClip =
        next.clip === 'Idle_Loop' && talkingRef.current ? 'Idle_Talking_Loop' : next.clip;
    ...
    const bodyShapeWidthScale =
      visual.bodyShape === 'broad' ? 1.12 : visual.bodyShape === 'slim' ? 0.92 : 1;
    const bodyScale = 1;
    ...
    return (
      ...
      <group ref={rootRef} scale={bodyScale}>
        ...
        <primitive
          object={scene}
          scale={[
            normalizeScale * skeletonWidthScale,
            normalizeScale,
            normalizeScale * skeletonWidthScale,
          ]}
        />
...
function buildHeadDecor(
  ...
  if (accessories.hat) {
    const brim = new THREE.Mesh(
      new THREE.CylinderGeometry(radius * 1.55, radius * 1.55, 0.018, 16),
      softToonMaterial('#e8e4dc')
    );
    brim.position.set(0, radius * 0.62, 0);
    const crown = new THREE.Mesh(
      new THREE.CylinderGeometry(radius * 0.92, radius * 1.0, radius * 0.95, 14),
      softToonMaterial('#e8e4dc')
    );
    crown.position.set(0, radius * 1.05, 0);
    const band = new THREE.Mesh(
      new THREE.CylinderGeometry(radius * 1.02, radius * 1.02, 0.03, 14),
      softToonMaterial(visual.accentColor)
    );
  ...
  if (accessories.tengu) {
    ...
    const muzzle = new THREE.Mesh(
      new THREE.CylinderGeometry(0.026, 0.026, 0.13, 8),
      softToonMaterial('#8a6a3c')
    );
  ...
  if (accessories.headband) {
    const bandeau = new THREE.Mesh(
      new THREE.CylinderGeometry(radius * 1.06, radius * 1.06, 0.035, 14, 1, true),
      softToonMaterial(visual.accentColor)
    );
    bandeau.position.set(0, 0.06, 0);
    group.add(bandeau);
    const plate = new THREE.Mesh(
      new THREE.BoxGeometry(0.07, 0.04, 0.012),
      softToonMaterial('#b8bcc8')
    );
  ...
  if (accessories.hairPin) {
    const pin = new THREE.Mesh(
      ...
      softToonMaterial(visual.accentColor, visual.accentColor)
    );
  ...
  if (accessories.glasses) {
    for (const side of [-1, 1]) {
      const lens = new THREE.Mesh(
        new THREE.TorusGeometry(0.034, 0.006, 6, 12),
        softToonMaterial('#2b3442')
      );
  ...
  if (visual.bodyShape === 'mechanical') {
    const visor = new THREE.Mesh(
      new THREE.BoxGeometry(radius * 2.1, 0.05, 0.05),
      softToonMaterial('#2b3442', visual.accentColor)
    );
  ...
  const cap = new THREE.Mesh(
    new THREE.SphereGeometry(radius * 1.12, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.58),
    hair.clone()
  );
  ...
  if (!noFringe.includes(style)) {
    const fringe = new THREE.Mesh(
      new THREE.SphereGeometry(radius * 0.58, 10, 8, 0, Math.PI * 2, 0, Math.PI * 0.5),
      hair
    );
