...
const DEFAULT_PROFILE: RoleProfile = {
  ...
  maxFloors: 3,
  density: 0.8,
  props: ['lamp', 'bench', 'crate'],
  trees: 3,
};

const ROLE_PROFILES: Array<{ pattern: RegExp; profile: RoleProfile }> = [
  {
    pattern: /urban|plaza|hub|city|square/i,
    profile: {
      minFloors: 2,
      maxFloors: 6,
      density: 0.9,
      props: ['lamp', 'bench', 'sign', 'planter'],
      trees: 2,
    },
  },
  {
  ...
  },
  {
    ...
    profile: {
      ...
      maxFloors: 2,
      density: 0.85,
      props: ['stall', 'sign', 'lamp', 'crate'],
      trees: 2,
    },
  },
  {
    pattern: /bridge|overpass|ruin|threat|alley|monster/i,
    profile: { minFloors: 1, maxFloors: 4, density: 0.5, props: ['crate', 'sign'], trees: 1 },
  },
];
...
function profileFor(location: Location): RoleProfile {
  return (
    ROLE_PROFILES.find((entry) => entry.pattern.test(roleTextFor(location)))?.profile ??
    DEFAULT_PROFILE
  );
}
...
function generateBuildings(
  ...
  const edges: Array<{
    ...
    {
      name: 'north',
      length: inner.maxX - inner.minX,
      horizontal: true,
      fixed: inner.minZ + beltDepth / 2,
      start: inner.minX,
    },
    {
      name: 'south',
      length: inner.maxX - inner.minX,
      horizontal: true,
      fixed: inner.maxZ - beltDepth / 2,
      start: inner.minX,
    },
    {
      name: 'west',
      length: inner.maxZ - inner.minZ - beltDepth * 2,
      horizontal: false,
      fixed: inner.minX + beltDepth / 2,
      start: inner.minZ + beltDepth,
    },
    {
      name: 'east',
      length: inner.maxZ - inner.minZ - beltDepth * 2,
      horizontal: false,
      fixed: inner.maxX - beltDepth / 2,
      start: inner.minZ + beltDepth,
    },
  ];
  ...
  for (const edge of edges) {
    ...
    while (cursor < edge.length) {
      ...
      if (!skip) {
        ...
        const body = hueShift(
          shiftColor(palette.structure, shade),
          range(rng, -18, 18),
          range(rng, 1.05, 1.3)
        );
        buildings.push({
...
function generateScatter(
  ...
  for (let i = 0; i < bushCount; i += 1)
    place('bush', i, shiftColor('#4e8f4a', range(rng, -0.1, 0.15)), palette.accent);

  const grassCount = 7 + Math.floor(rng() * 6); // 7-12
  for (let i = 0; i < grassCount; i += 1)
    place('grass', i, shiftColor('#6fae5c', range(rng, -0.1, 0.15)), palette.accent);
  ...
  for (let i = 0; i < rockCount; i += 1)
    place('rock', i, shiftColor(ROCK_COLOR, range(rng, -0.12, 0.1)), palette.accent);
